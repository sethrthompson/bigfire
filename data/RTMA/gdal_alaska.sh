#############################################################################################
#Title: gdal_alaska.sh
#Description: This is a program to complete the gdal process on each file that has completed
#             the degrib process in each sub directory (RT.00-RT.23) in AR.alaska folder.
...
#gdal_translate -of GTiff $file $file.tif && gdalwarp -t_srs '+proj=utm +datum=WGS84' -overwrite $file "$file".tif
